...
city_tier_mapping = module_from_file('utils','/home/airflow/dags/Lead_Scoring_Data_Pipeline/mapping/city_tier_mapping.py')
significant_categorical_level = module_from_file('utils','/home/airflow/dags/Lead_Scoring_Data_Pipeline/mapping/significant_categorical_level.py')
...
def build_dbs():
    ...
    SAMPLE USAGE
        build_dbs()
    '''
    print('DB Path' + constants.DB_PATH + constants.DB_FILE_NAME)
    if os.path.isfile(constants.DB_PATH + constants.DB_FILE_NAME):
...
def load_data_into_db():
    ...
    #For LeadScoring
    #df_lead_scoring = pd.read_csv(constants.DATA_DIRECTORY + 'leadscoring.csv')
    
    #For LeadScoring Inference
    df_lead_scoring = pd.read_csv(constants.DATA_DIRECTORY + 'leadscoring_inference.csv')
...
def map_categorical_vars():
    ...
    INPUTS
        ...
        **NOTE : list_platform, list_medium & list_source are all constants and
                 must be stored in '/mapping/significant_categorical_level.py'
    ...
    old_df = df_lead_scoring[df_lead_scoring['first_platform_c'].isin(
        significant_categorical_level.list_platform)]  # get rows for levels which are present in list_platform
    # concatenate new_df and old_df to get the final dataframe
    df = pd.concat([new_df, old_df])

    # get rows for levels which are not present in list_medium
    new_df = df[~df['first_utm_medium_c'].isin(significant_categorical_level.list_medium)]
    # replace the value of these levels to others
    new_df['first_utm_medium_c'] = "others"
    # get rows for levels which are present in list_medium
    old_df = df[df['first_utm_medium_c'].isin(significant_categorical_level.list_medium)]
    # concatenate new_df and old_df to get the final dataframe
    df = pd.concat([new_df, old_df])

    # get rows for levels which are not present in list_source
    new_df = df[~df['first_utm_source_c'].isin(significant_categorical_level.list_source)]
    # replace the value of these levels to others
    new_df['first_utm_source_c'] = "others"
    # get rows for levels which are present in list_source
    old_df = df[df['first_utm_source_c'].isin(significant_categorical_level.list_source)]
    # concatenate new_df and old_df to get the final dataframe
    df = pd.concat([new_df, old_df])
...
def interactions_mapping():
    ...
    if 'app_complete_flag' in df.columns:
        index_variable = constants.INDEX_COLUMNS_TRAINING
    else:
        index_variable = constants.INDEX_COLUMNS_INFERENCE
    ...
    df_model_input = df_pivot.drop(constants.NOT_FEATURES, axis=1)

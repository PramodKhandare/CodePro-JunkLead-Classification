...
def raw_data_schema_check():
    ...
    '''
    df_raw_data = pd.read_csv(constants.DATA_DIRECTORY + 'leadscoring.csv')

    check = set(df_raw_data.columns) == set(schema.raw_data_schema)
    if check:
...
def model_input_schema_check():
    ...
    check = set(df_model_input.columns) == set(schema.model_input_schema)
    if check:

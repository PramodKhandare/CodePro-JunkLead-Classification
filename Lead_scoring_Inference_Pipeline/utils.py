...
def encode_features():
    ...
    df_encoded.to_sql(name = 'feature_inference', con = cnx, if_exists = 'replace', index = False)
...
def get_models_prediction():
    ...
    # read the new data
    cnx = sqlite3.connect(constants.DB_PATH + constants.DB_FILE_NAME)
    df_new_data = pd.read_sql('Select * from feature_inference', cnx)
    ...
    df_new_data.to_sql(name='predicted_values', con = cnx, if_exists = 'replace', index = False)
    cnx.close()
...
def prediction_ratio_check():
    ...
    ct = datetime.now()
    st = str(ct) + ' %of 1 = ' + str(value_counts[1]) + ' %of 2 = ' + str(value_counts[0])
    with open(constants.FILE_PATH + 'prediction_distribution.txt', 'a') as f:
        f.write(st + "\n")
...
def input_features_check():
    ...
    # read the input data
    cnx = sqlite3.connect(constants.DB_PATH + constants.DB_FILE_NAME)
    df = pd.read_sql('Select * from feature_inference', cnx)

    # check if all columns are present
    check = set(df.columns) == set(constants.ONE_HOT_ENCODED_FEATURES)
